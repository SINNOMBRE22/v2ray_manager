...
# Path from where the script started
begin_path=$(pwd)

# Install way, 0 for fresh install, 1 to keep V2Ray config and update
install_way=0

# Define operation variables, 0 for no, 1 for yes
help=0
remove=0
chinese=0
...
# Temporary disable alias for CentOS
...
while [[ $# > 0 ]]; do
    ...
    case $key in
        ...
        -k|--keep)
        install_way=1
        colorEcho ${blue} "Keeping config to update\n"
        ;;
        --zh)
        chinese=1
        colorEcho ${blue} "Installing Chinese version..\n"
        ;;
        *)
        # unknown option
        ;;
...
removeV2Ray() {
    ...
    # Remove multi-v2ray environment variables
...
EOF
    ...
    # Remove old V2Ray bash_completion scripts
...
profileInit() {
    # Clean V2Ray module environment variables
    [[ $(grep v2ray ~/$env_file) ]] && sed -i '/v2ray/d' ~/$env_file && source ~/$env_file

    # Resolve Python3 Chinese display issue
